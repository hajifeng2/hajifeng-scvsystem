...
- 单人评估：[docs/评估框架/02-评估框架.md](docs/评估框架/02-评估框架.md) 的 8 维度 + 红旗 + 拒因 + 综合评估段（论述合不合适/为什么/关键判断/短板/验证点，评估卡第一节），基准 = 本地 `岗位画像_<职位>.md`
...
- **评估结果表**（待评估）：姓名 / 所属职位 / 评级(A\|B\|C+\|C) / `评估详情`（优势+风险+建议）/ `核心原因总结`（一句话合适/不合适原因）/ 评估日期 / 评估卡路径
- **moka简历库表**（简历库）：评估结果表字段 + `Moka面试结论`(通过\|待定\|淘汰\|未面) / `评估卡内容`（文字摘要，预留暂不写）。校准对比「评级(Agent 推荐) vs Moka 面试结论(真实标签)」

> 评级已含处置语义（A=推面试 / B=电话初筛 / C+=备选 / C=拒），不再单设「评估结论」列--避免与 Moka 真实标签混淆、避免 Agent 推荐被误读为业务结论。本地评估完成区三子目录（被接收/未通过/待定）仍保留，作为文件归属用。
> `评估详情`（优势+风险+建议，多行）+ `核心原因总结`（一句话）两层配套，step5 都写入；`评估卡内容`列预留暂不写。
